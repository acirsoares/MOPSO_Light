...
! @date Nov 28, 2022
! @author :!  Original FORTRAN77 version by Bennett Fox.
!             FORTRAN90 version by John Burkardt.
!             Object format adaptation by Acir M. Soares Jr.
! @def :  SobolSequence_mod 

!*****************************************************************************80
! This module introduces small adaptations to the original i4_sobol to 
! include some original functions and formats in a single object. 
!
...
!    FORTRAN90 version by John Burkardt.
!    Object format version by Acir M. Soares Jr.
!
! Initialization: 
!      TYPE(SobolSequenceGenerator_CLASS) :: "NAME"
!      "NAME"=SobolSequenceGenerator_CLASS(Number_of_dimensions)
!
! Function call:
!      ...="NAME"%i4_sobol()
!********************************************************************************
Module SobolSequence_mod
implicit none

  private

  public:: SobolSequenceGenerator_CLASS

  ! ** Class Definition
  type, public ::SobolSequenceGenerator_CLASS 
    
    ! ** Variables
    integer, private :: dim_num
    integer, private :: seed

  contains
  
    ! ** Class procedures
    procedure, public :: i4_sobol
    final :: DESTRUCTOR 
        
  end type SobolSequenceGenerator_CLASS  

  interface SobolSequenceGenerator_CLASS
    module procedure CONSTRUCTOR
  end interface SobolSequenceGenerator_CLASS  

contains
  ! ** Constructor
  !   Initialize The Sobol Sequence Generator (SSG)
  function CONSTRUCTOR(dim_num) result (SSG)
    implicit none
...
! ** Destructor
!   Destructs the Sobol Sequence Generator (SSG)
  subroutine DESTRUCTOR(SSG)
    implicit none
...
!   Generates a new quasirandom Sobol vector with each call.
  function i4_sobol (SSG) result(quasi)
!*****************************************************************************80
!
!  I4_SOBOL generates a new quasirandom Sobol vector with each call.
!
...
!
    ...
    class (SobolSequenceGenerator_CLASS) :: SSG
    ...
    real  quasi(SSG%dim_num)
    real  , save :: recipd
    integer , save :: seed_save = - 1
    integer  seed_temp
    integer , save, dimension(1:dim_max,1:log_max) :: v

    if ( .not. initialized .or. SSG%dim_num /= dim_num_save ) then
...
!
    ...
    if ( SSG%dim_num /= dim_num_save ) then
 
      !  Check parameters.
      if ( SSG%dim_num < 1 .or. dim_max < SSG%dim_num ) then
        write ( *, '(a)' ) ' '
        write ( *, '(a)' ) 'I4_SOBOL - Fatal error!'
        write ( *, '(a)' ) '  The spatial dimension DIM_NUM should satisfy:'
        write ( *, '(a,i8)' ) '    1 <= DIM_NUM <= ', dim_max
        write ( *, '(a,i8)' ) '  But this input value is DIM_NUM = ', SSG%dim_num
        stop
      end if

      dim_num_save = SSG%dim_num
      ...
      do j = maxcol - 1, 1, - 1
        l = 2 * l
        v(1:SSG%dim_num,j) = v(1:SSG%dim_num,j) * l
      end do
    ...
    if ( SSG%seed == 0 ) then
      ...
      l = 1
      lastq(1:SSG%dim_num) = 0

    else if ( SSG%seed == seed_save + 1 ) then

      !  Find the position of the right-hand zero in SEED.
      l = i4_bit_lo0 ( SSG%seed )

    else if ( SSG%seed <= seed_save ) then

      seed_save = 0
      l = 1
      lastq(1:SSG%dim_num) = 0

      do seed_temp = seed_save, SSG%seed - 1
        l = i4_bit_lo0 ( seed_temp )
        lastq(1:SSG%dim_num) = ieor ( lastq(1:SSG%dim_num), v(1:SSG%dim_num,l) )
      end do

      l = i4_bit_lo0 ( SSG%seed )

    else if ( seed_save + 1 < SSG%seed ) then

      do seed_temp = seed_save+1, SSG%seed - 1
        l = i4_bit_lo0 ( seed_temp )
        lastq(1:SSG%dim_num) = ieor ( lastq(1:SSG%dim_num), v(1:SSG%dim_num,l) )
      end do
    ...
    quasi(1:SSG%dim_num) = real ( lastq(1:SSG%dim_num)) * recipd
    lastq(1:SSG%dim_num) = ieor ( lastq(1:SSG%dim_num), v(1:SSG%dim_num,l) )
...
!
  ...
  end function i4_sobol

end Module SobolSequence_mod
